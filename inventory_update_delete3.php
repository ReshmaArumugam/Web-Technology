...
while($row)
{
    echo "<tr style=background-color:Gainsboro;text-align:center;'><td>$row[0]  </td><td>  $row[1]  </td><td>  $row[2] </td><td>  $row[3] </td><td><a href=inventory_update_delete4.php?pname=$row[0]&id=$row[1]&qty=$row[2]&sname=$row[3]>Update</a></td></tr> ";
    $row = mysqli_fetch_array($rs);
}
mysqli_close($conn);
?>  